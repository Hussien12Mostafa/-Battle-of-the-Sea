...
our::Texture2D *our::texture_utils::empty(GLenum format, glm::ivec2 size)
{
    ...
    texture->bind();
    int levels = 1;
    //passing the target , levels , width , hight to glTexStorage2D
...
our::Texture2D *our::texture_utils::loadImage(const std::string &filename, bool generate_mipmap)
{
    ...
    //bind the texture then fill the texture, the function takes the base mipmap level, the source format, the window size, the destination format, type of content, the source of the texture
    texture->bind();
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, size.x, size.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);
    
    //generate mipmap if we should
    if (generate_mipmap)
